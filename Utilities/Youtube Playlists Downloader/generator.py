...
def makeup(link):
    for c in range(len(link)):
        if link[c]=='l' and link[c+1]=='i' and link[c+2]=='s' and link[c+3]=='t' and link[c+4]=='=':
            link = link[c+5:]
            break

    for c in range(len(link)):
        if link[c]=='&' and link[c+1]=='i' and link[c+2]=='n' and link[c+3]=='d':
            link = link[:c]
            break
    return link
...
for itr in range(1,1+num):
    with open('..'+str(itr).zfill(l)+'-'+link+'-'+str(itr).zfill(l)+'.bat',mode='w+') as f:
        f.write('youtube-dl.exe www.youtube.com/playlist?list='+link+' --playlist-items '+str(itr)+' -o '+str(itr).zfill(l)+'-%'+'%'+'(title)s.'+'%'+'%'+'(ext)s -f "best[ext=mp4][height<='+str(quality)+']"  --external-downloader \
"aria2c.exe" --external-downloader-args "--max-concurrent-downloads 8 -x4 --log-level=error --show-console-readout false --summary-interval 5 --quiet=true"')
    if itr%10==0:
